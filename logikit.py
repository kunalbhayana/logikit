...
def make_open_action(label, path):
    """A profileAction that opens a file, folder or URL via the shell."""
    name = f"$@Generic___@ProfileAction___{guid()}"
    return name, {
        "$type": "Loupedeck.Service.ApplicationProfileCommand, "
                 "LoupedeckService",
        "isCommand": True,
        "name": name,
        "templateActionName": "$@Generic___@ShellExecute",
        "actionParameters": {
            "$type": "Loupedeck.ActionEditorActionParameters, PluginApi",
            "parameters": {
                "$type": "Loupedeck.StringDictionaryNoCase, PluginApi",
...
GLYPHS = {
    ...
    "close": _svg(
        '<path fill="#fff" d="M7.2 5.1a1.5 1.5 0 0 0-2.1 2.1L13.9 16l-8.8 8.8a'
        '1.5 1.5 0 1 0 2.1 2.1L16 18.1l8.8 8.8a1.5 1.5 0 0 0 2.1-2.1L18.1 16l8'
        '.8-8.8a1.5 1.5 0 0 0-2.1-2.1L16 13.9 7.2 5.1Z"/>'),
    "mail": _svg(
        '<path fill="#fff" d="M3 9.4V24a3 3 0 0 0 3 3h20a3 3 0 0 0 3-3V9.4l-1'
        '2.4 8.3a3 3 0 0 1-3.2 0L3 9.4Z"/><path fill="#fff" d="M29 6.2A3 3 0 0'
        ' 0 26 4H6a3 3 0 0 0-3 2.2l13 8.7 13-8.7Z"/>'),
    "play": _svg(
...
def cmd_build(args):
    ...
    for gi, group in enumerate(groups):
        ...
        for slot, item in enumerate(group):
            ...
            one = steps[0].strip()
            if len(steps) == 1 and one.lower().startswith("open:"):
                ref, action = make_open_action(item["label"], one[5:].strip())
